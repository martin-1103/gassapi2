...
import { ImportTypeInfo } from '@/utils/import/types';
...
export const ImportFormatSelector: React.FC<ImportFormatSelectorProps> = ({
  ...
  const selectedFormat = formatOptions.find(option => option.value === importType);
      ...
      {selectedFormat && (
        <div className="p-4 bg-muted/50 rounded-lg">
          ...
          <div className="text-xs">
            ...
            <p className="mt-1 text-muted-foreground">{selectedFormat.example}</p>
          </div>
        </div>
      )}
    </div>
