...
export function ResponseHeadersTab({ headers }: ResponseHeadersTabProps) {
  ...
  const categorizeHeader = (key: string): string => {
    ...
    // Cookie headers
    if (keyLower.includes('cookie') || keyLower === 'set-cookie')
      return 'Cookies';
    ...
    if (
      ...
      keyLower.startsWith('strict-transport-security') ||
    ...
    // Server info headers
    if (
      keyLower.includes('server') ||
      keyLower.includes('x-powered-by') ||
      keyLower === 'via'
    )
      return 'Server';

    // Encoding headers
    if (keyLower.includes('encoding') || keyLower.includes('compression'))
      return 'Encoding';
  ...
  categories.forEach(category => {
    const categoryHeaders = filteredHeaders.filter(
      ([key]) => categorizeHeader(key) === category,
    );
  ...
  const copyCategoryHeaders = (category: string) => {
    const foundCategory = groupedHeaders.find(cat => cat.name === category);
    if (!foundCategory) return;

    const headersText = foundCategory.headers
      .map(({ key, value }) => `${key}: ${value}`)
      .join('\n');
    navigator.clipboard.writeText(headersText);
    toast({
    ...
    <div className='h-full flex flex-col'>
      ...
      <div className='flex items-center justify-between p-4 border-b'>
        <div className='flex items-center gap-3'>
          ...
          <Badge variant='outline'>{filteredHeaders.length} shown</Badge>
        </div>
            ...
            {groupedHeaders.map(category => (
              <div key={category.name} className='bg-muted/30'>
                ...
                <div className='px-4 py-2 flex items-center justify-between bg-muted/50'>
                  ...
                  <div className='flex items-center gap-2'>
                    ...
                    <Button
                      size='sm'
                      variant='ghost'
                      onClick={() => copyCategoryHeaders(category.name)}
                    >
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className='w-1/3 font-medium text-xs'>
                        Header Name
                      </TableHead>
                      <TableHead className='w-2/3 font-medium text-xs'>
                        Value
                      </TableHead>
                      <TableHead className='w-auto font-medium text-xs'>
                        Description
                      </TableHead>
                    </TableRow>
                  </TableHeader>
      ...
      <div className='p-4 border-t bg-muted/50'>
        <div className='flex items-center justify-between text-xs text-muted-foreground'>
          ...
          <div className='flex items-center gap-4'>
            <span>
              Essential headers:{' '}
              {Object.keys(headers).filter(isEssentialHeader).length}
            </span>
