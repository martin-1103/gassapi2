 ...
 */
export const parsePostmanCollection = async (
  content: string,
  ...
  try {
    ...
    if (!parsed.info || !parsed.info.name) {
      throw new Error(
        'Format Postman collection tidak valid - harus ada info.name',
      );
    }

    if (!parsed.item || !Array.isArray(parsed.item)) {
      throw new Error(
        'Format Postman collection tidak valid - harus ada array item',
      );
    }
    ...
    const transformedData = {
      ...
      info: parsed.info,
      collections: collections.map((item: PostmanItem, index: number) => ({
        ...
        item: item.item
          ? item.item
              ...
              .map((endpoint: PostmanItem, endpointIndex: number) => ({
                ...
                params:
                  endpoint.request?.url?.query?.reduce(
                    (acc: Record<string, string>, param: PostmanQueryParam) => {
    ...
    const totalEndpoints = collections.reduce(
      (total: number, collection: { item?: unknown[] }) =>
        total + (collection.item?.length || 0),
      0,
    );
    ...
    return {
      success: false,
      message: `Gagal parse Postman collection: ${errorMessage}`,
      warnings: [
        'Pastikan file adalah Postman Collection v2.1 format yang valid',
      ],
    };
  }
};
