 ...
 */
export function useDirectApi(variableContext?: VariableContext): UseDirectApiReturn {
    ...
    try {
      ...
      try {
        await requestHistory.addItem({
          ...
          collectionId: config.body?.type === 'json' ? (config.body.json as any)?.collectionId : undefined,
        })
      } catch (historyError) {
...
export function useQuickRequest() {
  ...
  const quickRequest = useCallback(async (
