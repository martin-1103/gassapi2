...
import {
  ...
  getValidHeadersForRequest,
} from '@/lib/validations/header-validation';
...
export function useHeadersState({
  ...
  const addCommonHeader = useCallback(
    (commonHeader: (typeof COMMON_HEADERS)[0]) => {
      const newHeader = createHeader({
  ...
  const duplicateExistingHeader = useCallback(
    (id: string) => {
  ...
  // Utility functions
  const getHeadersObject = useCallback(
  ...
  // Search/filter
  const searchHeaders = useCallback(
    (query: string) => {
  ...
  const applyTemplate = useCallback(
    (templateHeaders: Partial<RequestHeader>[]) => {
