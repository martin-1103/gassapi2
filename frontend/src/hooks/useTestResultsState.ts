 ...
 */
export function useTestResultsState(
  ...
  const filters = useMemo(
    () => ({
      ...
      getSlowestTests: (limit?: number) =>
        sortTestsByDuration(testResults, 'desc').slice(0, limit),
      getFastestTests: (limit?: number) =>
        sortTestsByDuration(testResults, 'asc').slice(0, limit),
    }),
    [testResults],
  );
