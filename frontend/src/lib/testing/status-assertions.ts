...
export interface IStatusAssertions {
  ...
  toHaveStatusInRange(start: number, end: number, message?: string): AssertionResult;
...
export class StatusAssertions implements IStatusAssertions {
  ...
  toHaveStatus(expected: number, message?: string): AssertionResult {
    ...
    return addAssertion(this.context, 'to have status', {
      expected,
      actual: status,
      passed
    }, assertionMessage);
  ...
  toHaveStatusInRange(start: number, end: number, message?: string): AssertionResult {
    ...
    return addAssertion(this.context, 'to have status in range', {
      expected: `${start}-${end}`,
      actual: status,
      passed
    }, assertionMessage);
