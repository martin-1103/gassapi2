...
export class JsonAssertions implements IJsonAssertions {
  ...
  toHaveProperty(propertyPath: string, message?: string): AssertionResult {
    ...
    const assertionMessage =
      message ||
      (hasProperty
        ? `Has property "${propertyPath}"`
        : `Missing property "${propertyPath}"`);
  ...
  toHavePropertyWithValue(
    ...
    const assertionMessage =
      message ||
      (passed
        ? `Property "${propertyPath}" has expected value`
        : `Expected "${expectedValue}" but got "${actualValue}"`);

    return addAssertion(
      this.context,
      'to have property with value',
      {
  ...
  toBeObject(message?: string): AssertionResult {
    ...
    const assertionMessage =
      message || (passed ? 'Is object' : 'Value is not an object');
  ...
  expectArray(message?: string): AssertionResult {
    const hasDataProperty = this.toHaveProperty('data', message);
    const actualObj = this.actual as JsonData;
    const isArrayResult = new ContentAssertions(
      actualObj?.data,
      this.context,
    ).toBeArray();
