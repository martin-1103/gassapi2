 ...
 */
export const filterEntriesWithData = (
  entries: ConsoleEntry[],
): ConsoleEntry[] => {
  return entries.filter(
    entry => entry.data !== undefined && entry.data !== null,
  );
};
 ...
 */
export const filterEntriesWithStackTrace = (
  entries: ConsoleEntry[],
): ConsoleEntry[] => {
  return entries.filter(
    entry => entry.stackTrace && entry.stackTrace.trim().length > 0,
  );
};
