import type { ImportValidationResult } from '@/types/error-types';
  ...
  try {
    ...
    if (parsed.auth) {
      result.warnings.push(
        'Authentication settings in Postman collection may not be imported',
      );
    }
    ...
    result.errors.push(`Invalid JSON format: ${errorMessage}`);
    return result;
  }
};
...
export const validateOpenAPISpec = (
  ...
  try {
    const parsed: OpenAPISpec = JSON.parse(content);

    // Check if it's a valid OpenAPI/Swagger spec
    if (!parsed.openapi && !parsed.swagger) {
      result.isValid = false;
      result.errors.push(
        'Missing required "openapi" or "swagger" field in OpenAPI specification',
      );
      return result;
    }
    ...
    if (parsed.openapi) {
      ...
      if (!version.startsWith('3.')) {
        result.warnings.push(
          `OpenAPI version ${version} may not be fully supported. Recommended: 3.x.x`,
        );
      }
      ...
      if (!version.startsWith('2.')) {
        result.warnings.push(
          `Swagger version ${version} may not be fully supported. Recommended: 2.0`,
        );
      }
    }
...
export const validateCurlCommand = (
  ...
  if (!content.includes('curl ')) {
    result.isValid = false;
    result.errors.push(
      'Content does not appear to be a cURL command. Should start with "curl "',
    );
    return result;
  }
  ...
  if (!methodMatch) {
    result.warnings.push(
      'No HTTP method specified (use -X GET, -X POST, etc.)',
    );
  }
