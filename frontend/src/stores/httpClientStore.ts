...
export const useHttpClientStore = create<HttpClientStore>()(
  persist(
    (set, get) => ({
      ...
      setFollowRedirects: (follow: boolean) =>
        set({ followRedirects: follow }),

      setValidateSSL: (validate: boolean) =>
        set({ validateSSL: validate }),

      // History settings
      setSaveHistory: (save: boolean) =>
      ...
      // Display settings
      setPrettyPrintJson: (pretty: boolean) =>
        set({ prettyPrintJson: pretty }),
      ...
      setAutoSaveRequests: (save: boolean) =>
        set({ autoSaveRequests: save }),

      // Environment
      setEnvironment: (env: RuntimeEnvironment) =>
        set({ environment: env }),
