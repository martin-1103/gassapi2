...
export class VariableInterpolator {
    ...
    static interpolate(text, variables) {
        ...
        try {
            ...
            throw new ExecutionError(`Variable interpolation failed: ${error instanceof Error ? error.message : String(error)}`, ErrorCodes.VARIABLE_INTERPOLATION_ERROR, undefined, error);
        }
    }
    /**
     * Interpolate variables in an object recursively
     */
    static interpolateObject(obj, variables) {
     ...
     */
    static interpolateBody(body, variables) {
        if (!body)
            return body;
     ...
     */
    static interpolateUrl(url, variables) {
        return this.interpolate(url, variables);
    }
    /**
     * Interpolate variables in headers
     */
    static interpolateHeaders(headers, variables) {
        const result = {};
    ...
    static evaluateExpression(expression, context) {
        try {
            ...
            throw new ExecutionError(`Expression evaluation failed: ${error instanceof Error ? error.message : String(error)}`, ErrorCodes.CONDITION_EVALUATION_ERROR, undefined, error, { expression });
        }
    }
    /**
     ...
     */
    static validateContext(variables) {
        const errors = [];
...
VariableInterpolator.VARIABLE_REGEX = /\{\{(\w+)\}\}/g;
VariableInterpolator.ALLOWED_GLOBALS = {
    Math: Math,
    Date: Date,
    JSON: JSON,
    parseInt: parseInt,
