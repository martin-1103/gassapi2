...
export class FlowTools {
    ...
    async executeFlow(args) {
        try {
            ...
            // Execute flow
            const result = await this.flowExecutor.executeFlow(args.flowId, args.environmentId, args.overrideVariables, args.maxExecutionTime);
            // Format result
     ...
     */
    validateFlowExecutionArgs(args) {
        const errors = [];
     ...
     */
    formatFlowExecutionResult(result, debugMode) {
        ...
        if (result.executionPath && result.executionPath.length > 0) {
            ...
            result.executionPath.forEach((nodeId, index) => {
                const nodeResult = nodeResults.find((r) => r.nodeId === nodeId);
        ...
        if (debugMode && nodeResults.length > 0) {
            ...
            nodeResults.slice(0, 5).forEach((nodeResult) => {
