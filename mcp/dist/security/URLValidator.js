...
export class URLValidator {
    ...
    static validate(url, options = {}) {
        ...
        try {
            ...
            if (options.allowedDomains && options.allowedDomains.length > 0) {
                const isAllowed = options.allowedDomains.some(domain => parsedUrl.hostname === domain || parsedUrl.hostname.endsWith('.' + domain));
                if (!isAllowed) {
            ...
            if (options.blockedDomains && options.blockedDomains.length > 0) {
                const isBlocked = options.blockedDomains.some(domain => parsedUrl.hostname === domain || parsedUrl.hostname.endsWith('.' + domain));
                if (isBlocked) {
     ...
     */
    static hasSuspiciousPatterns(url) {
...
URLValidator.LOCALHOST_PATTERNS = [
    'localhost',
    '127.0.0.1',
    '::1',
    '0.0.0.0'
];
URLValidator.PRIVATE_IP_RANGES = [
    /^10\./, // 10.0.0.0/8
