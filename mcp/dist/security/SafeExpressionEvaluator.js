...
export class SafeExpressionEvaluator {
    ...
    static evaluate(expression, context = {}) {
        try {
            ...
            if (!validation.isValid) {
                throw new ExecutionError(`Expression validation failed: ${validation.errors.join(', ')}`, ErrorCodes.UNSAFE_EXPRESSION);
            }
            // Sanitize expression
            ...
            throw new ExecutionError(`Expression evaluation failed: ${error instanceof Error ? error.message : String(error)}`, ErrorCodes.CONDITION_EVALUATION_ERROR, undefined, error);
        }
    }
    /**
     ...
     */
    static createSafeContext(userContext) {
    ...
    static sanitizeContextValue(value) {
        ...
        if (Array.isArray(value)) {
            return value.filter(item => typeof item === 'string' || typeof item === 'number' || typeof item === 'boolean');
        }
     ...
     */
    static evaluateWithTimeout(expression, context) {
     ...
     */
    static testExpression(expression) {
        try {
            const validation = this.validateExpression(expression);
            if (!validation.isValid) {
     ...
     */
    static getAllowedGlobals() {
        return Object.keys(this.ALLOWED_GLOBALS);
    }
}
SafeExpressionEvaluator.ALLOWED_GLOBALS = {
    Math: Math,
    Date: Date,
    JSON: JSON,
    parseInt: parseInt,
...
SafeExpressionEvaluator.MAX_EXPRESSION_LENGTH = 1000;
SafeExpressionEvaluator.MAX_EXECUTION_TIME = 5000; // 5 seconds
