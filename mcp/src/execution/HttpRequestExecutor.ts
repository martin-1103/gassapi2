...
import { HttpRequestConfig, HttpResponse, ExecutionError, ErrorCodes } from '../types/execution.types.js';
...
export class HttpRequestExecutor {
   ...
   */
  async execute(config: HttpRequestConfig): Promise<HttpResponse> {
   ...
   */
  async executeWithRetry(config: HttpRequestConfig, retries: number = 3): Promise<HttpResponse> {
    let lastError: Error;
   ...
   */
  private async makeRequest(config: HttpRequestConfig): Promise<HttpResponse> {
    ...
    try {
      ...
      const responseTime = this.calculateResponseTime(startTime);
      const executionError = this.handleRequestError(error as Error, config, responseTime);
   ...
   */
  private prepareBody(body: any, headers?: Record<string, string>): string | Buffer | undefined {
   ...
   */
  private async parseBody(response: Response, headers: Record<string, string>): Promise<any> {
   ...
   */
  private handleRequestError(error: Error, config: HttpRequestConfig, responseTime: number): ExecutionError {
